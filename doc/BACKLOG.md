...
| **`-write_ctd` / `-write_cwl` fail** | unknown until diagnosed | `FASTag -write_ctd <dir>` and `-write_cwl <dir>` exit **8** and write nothing: `Requested tool 'FASTag' does not exist!`. `OpenMS::ToolHandler` looks the tool up in a HARD-CODED registry of official TOPP tools (`ToolHandler.cpp:getTOPPToolList`, throw at `ToolHandler.cpp:235`) and a tool built outside the OpenMS tree is not in it. `-write_ini` is unaffected (13,331 bytes, exit 0), so the GUI's generated manifest and every INI workflow are fine. What breaks is CTD/CWL export — how KNIME, Galaxy and Nextflow wrappers consume a TOPP tool — so this is an interoperability gap for an externally-distributed tool, not cosmetic. **Pre-existing**, unrelated to the 2026-09-09 work that surfaced it. Constraint: any fix must work against a STOCK OpenMS (bioconda 3.5.0), because patching OpenMS is not an option here and PRs to OpenMS need explicit permission. Under diagnosis on `fix/write-ctd`; if it proves upstream-only, the outcome is a precise upstream ask, not a hand-rolled descriptor generator — a wrong CTD is worse than none. |
...
- **`-write_ctd` / `-write_cwl` are broken** — found 2026-09-09 while fixing the
  `--help` threads line, pre-existing and unrelated to it. Exit 8, nothing
  written, because OpenMS's ToolHandler only knows official TOPP tools. See
  the row in "Still open, sized".
